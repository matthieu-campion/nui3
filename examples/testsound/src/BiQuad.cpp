...
void BiQuad::Process(float* pOutSamples, const float* pInSamples)
{
...
void BiQuad::Process(float* pOutSamples, const float* pInSamples, uint32 SampleFrames)
{
  for (uint32 i = 0; i < SampleFrames; i++)
  {
    Process(pOutSamples, pInSamples);
    pOutSamples += mChannels;
    pInSamples += mChannels;
  }
}
