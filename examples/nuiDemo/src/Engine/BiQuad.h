...
public:
  ...
  void Process(std::vector<float*>& OutSamples, const std::vector<float*>& InSamples, uint32 SampleFrames);
...
private:
  ...
  void Process(std::vector<float*>& OutSamples, const std::vector<float*>& InSamples); // Process one sample frame
};
