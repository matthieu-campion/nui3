...
bool nuiAudioFifo::Start ()
{
  ...
  if (!mpAudioDevice)
  {
    NGL_OUT(_T("nuiAudioFifo::Start ERROR : could not retrieve audio device!\n"));
    return false;
  }
  
  NGL_OUT(_T("AUDIOFIFO start\n"));
  ...
  if (!res)
  {
    NGL_OUT(_T("nuiAudioFifo::Start ERROR : could not open audio device!\n"));
    return false;
  }
...
void nuiAudioFifo::Close ()
{
  ...
  //NGL_OUT(_T("nuiAudioFifo::Close ok\n"));
}
...
void nuiAudioFifo::OnStart ()
{
  ...
  // NGL_OUT(_T("AUDIOFIFO Reading Thread start\n"));
  ...
  // NGL_OUT(_T("AUDIOFIFO Reading Thread stoped\n"));
