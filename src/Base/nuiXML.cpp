...
nglSize xmlLexer::ReadLine (nglString& rLine, nglTextFormat* pFormat)
{
   ...
   *pFormat = eTextNone;
   ...
   while (mpStream->GetState() == eStreamReady)
   {
   nglTextFormat format = eTextNone;
   int parsed = 0, ending = 0, bytes;
   
   bytes = mpStream->Peek (buffer, BUFFER_SIZE, 1);
   ...
   *pFormat = format;
   ...
   mpStream->SetPos(parsed, eStreamForward);
   bytes_total += parsed;
   return bytes_total;
   }
   else
