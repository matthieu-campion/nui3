...
typedef struct
{
    ...
    int   raw;
    uLong saved_pos; /* the position of the stream when we are about to change the current file */
...
extern voidp ZEXPORT unzGetCurrentFile (unzFile file)
{
  ...
  s=(unz_s*)file;
  s->pfile_in_zip_read->saved_pos = s->z_filefunc.ztell_file(s->z_filefunc.opaque, s->filestream);
...
extern int ZEXPORT unzSetCurrentFile (unzFile file, voidp _file)
{
  ...
  if (file==NULL)
      return UNZ_PARAMERROR;
  s=(unz_s*)file;
  if (s->pfile_in_zip_read != (file_in_zip_read_info_s*) _file) // If we are allready on the right file let's do nothing! (but do it well ;) ).
  {
    s->pfile_in_zip_read = (file_in_zip_read_info_s*) _file;
