...
package
{
  ...
  public class LiquidTest extends Sprite
  {
    ...
    private var _color:ColorTransform = new ColorTransform(.5,.5,1,22);
    ...
    public function paint(event:Event):void
    {
      ...
      {
        _canvas.setPixel32(int(4.0 * p.x), int(4.0 * p.y), 0x80000080);
        _canvas.setPixel32(int(4.0 * (p.x - p.u)), int(4.0 * (p.y - p.v)), 0xFF0000FF);
      }
